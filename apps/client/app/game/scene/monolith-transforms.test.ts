...
describe( 'bodyTransform', () => {
    ...
    it( 'spans from below the floor to the full height', () => {
        const t = bodyTransform( box, { z: 0, side: 1 } );
...
describe( 'seamTransform', () => {
    it( 'sits on the chamfer of the inner face, toward the track', () => {
        const t = seamTransform( box, { z: 0, side: 1 } );
        expect( t.position[ 0 ] ).toBeCloseTo( innerFaceX( box ) + bevel );
    } );

    it( 'sits on the outer face when asked', () => {
        const t = seamTransform( withSeam( box, { face: 'outer' } ), { z: 0, side: 1 } );
    ...
    it( 'centers on the face, or rides the near or far chamfer', () => {
        ...
        expect( seamTransform( withSeam( box, { align: 'center' } ), { z: 500, side: 1 } ).position[ 2 ] ).toBe( 500 );
        expect( seamTransform( withSeam( box, { align: 'near' } ), { z: 500, side: 1 } ).position[ 2 ] ).toBeCloseTo(
            500 - edge,
        );
        expect( seamTransform( withSeam( box, { align: 'far' } ), { z: 500, side: 1 } ).position[ 2 ] ).toBeCloseTo(
            500 + edge,
        );
    } );

    it( 'faces the chamfer diagonal, and squares up mid-face', () => {
        const near = seamTransform( box, { z: 0, side: 1 } );
        ...
        expect( seamTransform( withSeam( box, { align: 'center' } ), { z: 0, side: 1 } ).rotationY ).toBe( 0 );
    } );

    it( 'mirrors the diagonal on the far side of the track', () => {
        const right = seamTransform( box, { z: 0, side: 1 } );
        const left = seamTransform( box, { z: 0, side: -1 } );
    ...
    it( 'leans with the taper, mirrored per side', () => {
        const right = seamTransform( obelisk, { z: 0, side: 1 } );
        const left = seamTransform( obelisk, { z: 0, side: -1 } );
    ...
    it( 'tracks the tapered face rather than the base face', () => {
        const t = seamTransform( obelisk, { z: 0, side: 1 } );
