...
export const SEEKER_FLIGHT: SeekerForm = {
    half: 0.55,
    halfLen: 4.4,
    nose: DEFAULT_SIM_CONFIG.seekerHalf,
    fins: true,
    faces: 1,
};
...
const FIN_SEAT = 0.02;
const FIN_ROLLS = [ 0, Math.PI / 2, Math.PI, -Math.PI / 2 ];
const FACE_LIFT = 0.006;

export function seekerTail( f: SeekerForm ): number {
    return 2 * f.halfLen - f.nose;
}
...
function place( g: THREE.BufferGeometry, f: SeekerForm ): THREE.BufferGeometry {
    return g.translate( 0, 0, f.nose - f.halfLen );
}
...
function fins( f: SeekerForm ): THREE.BufferGeometry[] {
    ...
    const shape = new THREE.Shape( [
        ...
        new THREE.Vector2( -l + FIN_ROOT, 0 ),
        new THREE.Vector2( -l + FIN_ROOT * 0.5, FIN_HEIGHT ),
        new THREE.Vector2( -l - FIN_ROOT * 0.05, FIN_HEIGHT ),
    ] );
    return FIN_ROLLS.map( ( roll ) =>
...
export function seekerShellGeometry( f: SeekerForm ): THREE.BufferGeometry {
    ...
    if ( ! f.fins ) return place( body, f );
    return place( merge( [ body, ...fins( f ) ] ), f );
}
...
export function seekerGlyphGeometry( f: SeekerForm ): THREE.BufferGeometry {
    ...
    return place( merge( [ rim, ...bands ] ), f );
}
