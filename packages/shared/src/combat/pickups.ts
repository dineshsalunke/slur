...
import { HALF_WIDTH, SEG_LEN, START_SAFE, TRACK_SEGMENTS } from '../sim/track.js';
...
// Deterministic pickup layout: one slot per PICKUP_SPACING segments after the start-safe zone, jittered
// laterally within the corridor (kept a grab-radius clear of the rails), centred forward in its segment.
// Seeded per slot via hash2(seed, seg) → a LOCAL mulberry32 (same random-access pattern as segmentAt).
