import {
    BLOCK_DEPTH_CURVE_K_PEAK,
    BLOCK_DEPTH_CURVE_K_REST,
    BLOCK_DEPTH_MAX,
    BLOCK_DEPTH_MIN,
    BLOCK_SPLIT_GAP_MAX,
    BLOCK_SPLIT_GAP_MIN,
    BLOCK_WIDTH_CURVE_K_PEAK,
    BLOCK_WIDTH_CURVE_K_REST,
    BLOCK_WIDTH_MAX,
    BLOCK_WIDTH_MIN,
} from '../constants.js';
...
function skew( u: number, k: number ): number {
    return u / ( u + k * ( 1 - u ) );
}

export function blockDepthFor( u: number, intensity: number, maxDepth: number ): number {
    const k = BLOCK_DEPTH_CURVE_K_REST + ( BLOCK_DEPTH_CURVE_K_PEAK - BLOCK_DEPTH_CURVE_K_REST ) * intensity;
    const t = skew( u < 0 ? 0 : u > 1 ? 1 : u, k );
...
export function carveRun( seed: number, i: number, key: number, runWidth: number, intensity: number ): number[][] {
    ...
    const k = BLOCK_WIDTH_CURVE_K_REST + ( BLOCK_WIDTH_CURVE_K_PEAK - BLOCK_WIDTH_CURVE_K_REST ) * intensity;
    const out: number[][] = [];
    ...
    while ( runWidth - x >= BLOCK_WIDTH_MIN ) {
        ...
        const w = BLOCK_WIDTH_MIN + ( top - BLOCK_WIDTH_MIN ) * skew( r(), k );
        out.push( [ x, x + w ] );
