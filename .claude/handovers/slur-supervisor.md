...
| workertwo | w2P:pF | #222 destructible block rebuild: IDLE; tuned `b83ac9a`, handover `d18b629`; stills in .claude/frame-tap-refs/222/ SENT to the owner. Owner Qs: a distance glow boost at 180u (1–2 px)? fix the one-frame gap between the block vanishing and the debris appearing? tuning-schema.ts released. Perf: fractured mesh 2 calls; a break adds 3 calls and 8.7k tris; JS 7.4 → 9.7 ms during a break (swiftshader). | Built `2ab2c2c` (12 cells, bolt pre-glow, analytic burst), ADR-015 amendment `fef4a21`, ART_MATERIALS item 15 `4589412`. Cleared and resumed. NOW: range stills, break-sequence taps, tuning, perf numbers, then stills to the owner. Typecheck scare CLOSED: workertwo ran the full `pnpm typecheck` at 17fede3 and it exits 0 (workerthree's report was probably taken mid-save). Tuning: the default cracks fade by ~100u; gap 0.3 + glow 12 reads at 50u; 0.45 + 18 blooms into a blob. | #222 scene files (fractured-block-geometry.ts(+test), fractured-block-shader.ts, block-debris.tsx, block-breaks.ts, block-burst.tsx, track-blocks.tsx, game/block-state.ts), dev/tuning-schema.ts (re-cleared) |
| workerthree | w2P:pG | #223 3 slots: BUILT end to end (738ebd3 client, c362fab decode fix, 9928cc1 DECISIONS; DECISIONS.md released). Cleared and resumed after 36dd249. NOW: live checks (a hosted race, the /test-level HUD still). The older detail follows: | Seeker 4x length + 4 fins `fcebd5d` (visual only; the nose sits on the sim front face). #223 shared + server landed as ONE commit `eb4c381`: server 13/13, shared 202/202. Hosted E does nothing until the client slice sends {slot}. NOW: ART_MATERIALS item 14 (8.8u, four fins), then the ADR-017 amendment, each in its own commit, then release both. Then the #223 client slice (keys, store, 3-cell HUD, audio, test-level). | combat/*, schema.ts, sim-config.ts, server rooms/{run-room.ts(+test), room-combat.ts}, seeker-*, test-level local-* + test-level-hud.tsx, hud/power-rack.tsx + power-cell.tsx (new), net-power-rack.tsx (new), net-debug-hud.tsx, overlays.test.tsx, deletes hud/power-slot.tsx + local-power-slot.tsx + held-power-chip.tsx, net-canvas.tsx, overlays/overlays.tsx, audio/bind-room-audio.ts, game/input/ (new), docs/GDD.md, docs/ART_MATERIALS.md, docs/DECISIONS.md |
...
1a. RULE FIX (owner approval): `@deprecated()` broke hosted decoding (eb4c381 → fixed c362fab). The client
