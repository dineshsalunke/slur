...
- **`/solo` was REMOVED** (commit `52f5a04`) and stays removed — the old S1 note "`pnpm dev` → /solo" is stale. **Playtest the track in a hosted room:** the S4 `/game/:roomId` path materializes + renders + `simulate()`s the real track, and a host can **start solo** (no min-player gate on `startRace()`) → GO → race. Controls: W/S throttle-brake, A/D strafe, Space jump (tap/hold/double), Shift boost. Tune flight via `DEFAULT_JUMP` + `DEFAULT_TUNING` in `packages/shared/src/constants.ts`; tune the track via the ADR-006 section-envelope + banks constants.
