...
- **Environment:** monoliths (Obelisk · Gate · Arch) and asteroids (Angular · Plate · Broken, varied by
  Elongated · Shattered · Cluster). Asteroids stay dark stone / worn concrete. **Monoliths and obstacle
  blocks use the deck material** (`docs/ART_MATERIALS.md` rev. 8, 2026-09-24). Hazard-vs-scenery
  separation for monoliths rests on scale and placement alone (the row below). A block differs from the
  deck by its seams, wear and form. Planets and moons stay
...
- **No atmosphere:** flat `#05060a` void — no fog, skybox/gradient, or real starfield (cuberun, our anchor, leans on fog + a ~10k-star field + a galaxy skybox for depth). *(STALE 2026-09-22 — the scene now carries a nebula backdrop, a drei starfield and linear scene fog (`scene/scene-fog.tsx`, issue #210, `Fog.*` in the tuning panel). Since 2026-09-24 (#224) there is no scene fog at all, on the owner's call: the horizon-tinted fog at 40–420u greyed the far monoliths while the sky and rocks behind them stayed clear, which read as haze in a vacuum. Blocks now pop in at the streaming edge `AHEAD = 900`, where an 8u block is under a pixel.)*
